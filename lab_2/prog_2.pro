...
clauses
    ...
    %Возраст сотрудника в момент начала работы
    ...
    %Иерархия сотрудников заданного отдела
    ...
    %Печать информации о сотруднике по идентификатору
    employee(IDemployee) :-
    ...
    %Найдем сотрудников заданного отдела
    department_employee(Name) :-
        ...
        nl,
        fail.
        %Если сотрудник не найден в отделе
    department_employee(Name) :-
        department(_, Name).

    %Посчитаем сумму заработной платы в заданном отделе
    department_sal(Name) :-
        department(DepartmentId, Name),
        works(EmployeeId, PositionId, _),
